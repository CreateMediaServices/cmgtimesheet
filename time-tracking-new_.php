...
while ( $row = $result->fetch_assoc() ):
	...
	$projectTypeLabel[$counter] = $row[ "projectType" ];	
	$counter++;
endwhile;
...
while ( $row = $result->fetch_assoc() ):
	...
	$jobTypeName[$counter] = $row[ "jobTitle" ];	
	$counter++;
endwhile;
...
$sql = "SELECT * FROM cmg_clients where clientStatus=0 ORDER BY clientName";
$result = $db->query( $sql );
$totalRecords = $result->num_rows;
$counter = 0;
...
?>
		<tr>
			<td class="clientName"><?php echo $uClientName[$clientIDSelect[ $counter ]]; ?></td>
			<td class="jobType"><?php echo $jobTypeName[$jobTypeIDSelect[$counter]-1]; ?></td>			
