...
if($current == 0){
	...
	$sqlTime = "SELECT cmg_clients.id, cmg_clients.clientName, 
					sum(cmg_timetracker.jobHours) as SumJobHours
					FROM cmg_clients JOIN cmg_timetracker
				 	ON cmg_clients.id = cmg_timetracker.clientID				 	
				 	WHERE (cmg_timetracker.jobDate >='".$startDate."' AND
					cmg_timetracker.jobDate <='".$endDate."')
					GROUP BY cmg_clients.clientName
...
while ( $rowTime = $resultTime->fetch_assoc() ):
	$clientName[ $counter ] = $rowTime[ "clientName" ];
	$clientID[ $counter ] = $rowTime[ "id" ];
...
<?php 
$counter=0;
while($counter < sizeof($clientName) ) :
	$detailURL = "clientDetail.php?clientID=".$clientID[$counter]
					.'&clientName='.$clientName[$counter].$queryString;
?>
			<tr>
				<td class="clientName">
					<a href="<?= $detailURL; ?>">
						<?php echo($clientName[$counter]); ?>
					</a>
				</td>
				<td class="jobHours">
