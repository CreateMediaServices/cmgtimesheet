...
while ( $row = $result->fetch_assoc() ):
	...
	$uDeptName[$uDeptID[$counter]] = $row[ "deptName" ];
	$counter++;
endwhile;

$sql = "SELECT * FROM cmg_clients WHERE clientStatus=0";
$result = $db->query( $sql );
$totalRecords = $result->num_rows;
$counter = 0;
...
$sql = "SELECT * FROM cmg_jobtypes WHERE jobStatus=0";
$result = $db->query( $sql );
$totalRecords = $result->num_rows;
$counter = 0;

while ( $row = $result->fetch_assoc() ):
	$uJobID[$counter] = $row[ "id" ];
	$uJobDID[$counter] = $row[ "deptID" ];
	$uJobClientID[$counter] = $row[ "clientID" ];
	$uJobTitle[$counter] = $row[ "jobTitle" ];
	$counter++;
endwhile;
                                        ...
                                        <div class="row">
                                        	...
                                        	<div class="col-md-6">
                                                <div class="c-form-item">
                                                    ...
                                                    <select style="width: 100%" name="jobID" id="jobID">
                                                    	<option value="0">Select client</option>
<?php 

	$counter=0;
	while($counter < sizeof($uJobTitle) ) :
		$Label = $uDeptName[$uJobDID[$counter]].' - '
				.$uClientName[$uJobClientID[$counter]].' - '
				.$uJobTitle[$counter];
?>
														<option value="<?php echo $uJobID[$counter]; ?>"><?= $Label; ?></option>
<?php
		$counter++;
	endwhile;
?>
                                            ...
                                            <div class="col-md-3">
                                                <div class="c-form-item">
                                                  	<label for="clients">&nbsp;</label>                                                   	
                                                   	<a href="javascript:void(0);" class="c-btn btn--emp js-remove-jobtype"><i class="fa"></i>Remove Job Title</a>                                                    
                                                    <input type="hidden" name="siteBaseURL" id="siteBaseURL" value="<?php echo($siteBaseURL.'/removeJobType-new.php'); ?>">													
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
                                        	<div class="col-md-12">
                                        		<div class="success">Job Type removed</div>
